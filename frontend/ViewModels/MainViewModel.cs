...
public partial class MainViewModel : ViewModelBase, IDisposable
{
    ...
    private async Task ExportWavAsync()
    {
        try
        {
            ...
            {
                StatusMessage = "Error: Cannot open file dialog.";
                return;
            }
            ...
            bool success = await _ipcService.SendCommandAsync("exportWav", Guid.NewGuid().ToString(),
                new { path });

            StatusMessage = success ? $"Exported: {path}" : "Error: Export failed.";
        }
        catch (Exception ex)
        {
            StatusMessage = $"Error: {ex.Message}";
