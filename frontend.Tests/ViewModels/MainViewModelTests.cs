...
public class MainViewModelTests
{
    ...
    public async Task PlayAsync_WhenAlreadyPlaying_DoesNotSendCommand()
    {
        ...
        await vm.PlayCommand.ExecuteAsync(null); // first play
        await vm.PlayCommand.ExecuteAsync(null); // second play — should be ignored
    ...
    public async Task ToggleLoop_OnFailure_RevertsState()
    {
        ...
        Assert.Equal("Loop ↩", vm.LoopButtonText); // unchanged
    }
    ...
    {
        var (vm, _) = Build();
        await vm.SetLfoShapeCommand.ExecuteAsync("square"); // change first
        await vm.SetLfoShapeCommand.ExecuteAsync("sine");   // then back to sine
    ...
    {
        var (vm, _) = Build(ipcReturns: true);
        await vm.AddNoteAsync(60, 0); // 1 note

        await vm.DeleteNoteAsync("nonexistent-id");

        Assert.Single(vm.Notes); // still 1
    }
}
